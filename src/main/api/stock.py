...
def fuzzy_best_match(text , vig_class_id):
    meds_file_path = ""
    if vig_class_id == 0:
...
results = model.predict(source=src,conf=0.7,show=False, save_crop=False , save = False,retina_masks=True )
    ...
    if (color_id==0) or (color_id==2):
        ...
        for r in results:
                ...
                results_NDD = model_champs_NAME_DATE_DOS.predict(source=output_path, show=False, save=False,
                                                                ...
                                                                ,retina_masks=True,save_conf=True)

                results_PF = model_champs_PPA_FORM.predict(source=output_path, show=False, save=False, 
                                                            show_labels=False, show_conf=False, conf=0.1, 
                                                            save_txt=False, save_crop=False, retina_masks=True,save_conf=True )
...
print(segmented_image_without_ppa_date_to_text(f'./{directoryPath}/without_Date_PPA.png'))
